...
_lpips_model = lpips.LPIPS(net='vgg').requires_grad_(False)

def calculate_lpips(img1, img2):
    with torch.no_grad():
        _lpips_model.to(img1)
        lpips_value = _lpips_model.forward(img1, img2, normalize=True)
    return lpips_value.item()


_dists_model = DISTS().eval().requires_grad_(False)

def calculate_dists(img1, img2):
    with torch.no_grad():
        _dists_model.to(img1)
        dists_value = _dists_model(img1, img2)
    return dists_value.item()


_vif = VisualInformationFidelity().eval().requires_grad_(False)

def calculate_vif(img1, img2):
    with torch.no_grad():
        _vif.to(img1)
        return _vif.forward(img1, img2).item()
