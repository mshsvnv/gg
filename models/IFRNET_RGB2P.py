...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # Все каналы уменьшены в 2 раза (32->16, 48->24, 72->36, 96->48)
...
class Decoder4(nn.Module):
    def __init__(self):
        ...
        # Вход: 48+48+1=97. Выход: 40 (36 feature + 4 flow)
        self.convblock = nn.Sequential(
            convrelu(48+48+1, 48), 
            ResBlock(48, 16), # side_channels тоже уменьшены (32->16)
...
class Decoder3(nn.Module):
    def __init__(self):
        ...
        # Вход: 36 (ft_) + 36 + 36 + 2 + 2 = 112. Выход: 28 (24 feature + 4 flow)
        self.convblock = nn.Sequential(
            convrelu(112, 108), 
            ResBlock(108, 16), 
...
class Decoder2(nn.Module):
    def __init__(self):
        ...
        # Вход: 24 (ft_) + 24 + 24 + 2 + 2 = 76. Выход: 20 (16 feature + 4 flow)
        self.convblock = nn.Sequential(
            convrelu(76, 72), 
            ResBlock(72, 16), 
...
class Decoder1(nn.Module):
    def __init__(self):
        ...
        # Вход: 16 (ft_) + 16 + 16 + 2 + 2 = 52. Выход: 8 (остается 8: 4 flow + 1 mask + 3 res)
        self.convblock = nn.Sequential(
            convrelu(52, 48), 
            ResBlock(48, 16), 
            nn.ConvTranspose2d(48, 8, 4, 2, 1, bias=True)
        )
...
class Model(nn.Module):
    def __init__(self, local_rank=-1, lr=1e-4):
        ...
        self.perc_loss = LPIPS(net='vgg').requires_grad_(False)
    ...
    def forward(self, img0, img1, embt, imgt, flow=None):
        ...
        self.perc_loss.to(imgt_pred)
        loss_perc = self.perc_loss.forward(imgt_pred, imgt, normalize=True).mean()
        loss_rec = self.l1_loss(imgt_pred - imgt) + self.tr_loss(imgt_pred, imgt) + 0.1 * loss_perc
